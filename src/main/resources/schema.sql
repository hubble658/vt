-- =====================================================
-- StudyFlow Veritabanı Şeması
-- PostgreSQL için Gelişmiş Özellikler
-- =====================================================
...
-- Reservations tablosuna silme kısıtı: İptal edildikten sonra silinemesin diye
...
-- =====================================================
-- 5. SQL FONKSİYONLARI (3 Adet - Biri Cursor ile)
-- =====================================================
...
-- FONKSİYON 2: En Uygun Zaman Önerisi (CURSOR Kullanan Fonksiyon)
...
-- FONKSİYON 3: Kullanıcı Rezervasyon Geçmişi Raporu (RECORD Kullanan Fonksiyon)
...
CREATE OR REPLACE FUNCTION fn_get_popular_facilities(
    p_min_reservations INTEGER DEFAULT 5
) RETURNS TABLE (
    facility_id BIGINT,
    facility_name VARCHAR,
...
BEGIN
    ...
    SELECT 
        ...
        ROUND(
            COUNT(CASE WHEN r.status = 'CANCELLED' THEN 1 END)::DECIMAL / 
    ...
    LEFT JOIN reservations r ON r.seat_id = s.id
    GROUP BY f.id, f.name
    HAVING COUNT(r.id) >= p_min_reservations
