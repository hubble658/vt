...
public class UserReservationStep1Controller {
    ...
    @FXML private HBox dayContainer;
    @FXML private VBox weeklyScheduleInfoBox;
    @FXML private Label errorLabel;

    // Time Picker kutularındaki label'lar
    @FXML private Label startTimeDisplay;
    @FXML private Label endTimeDisplay;

    // Overlay (büyük saat seçici)
    @FXML private VBox timeOverlay;
    @FXML private Label overlayTitleLabel;
    @FXML private VBox overlayTimesBox;

    // YENİ: Öneri Kutusu
    @FXML private HBox suggestionBox;
    @FXML private Label lblSuggestionText;
    ...
    @Autowired private ReservationService reservationService; // Servis Eklendi

    private Facility facility;
    private ToggleGroup dayGroup;
    private LocalDate selectedDate;

    private enum TimeMode { START, END }
    ...
    private ToggleButton createDayButton(LocalDate date, boolean isOpen) {
        ...
        if (date.equals(LocalDate.now())) dayName = "Today";
        ...
        if (!isOpen) {
            btn.setDisable(true);
            Label lblClosed = new Label("CLOSED");
    ...
    private void renderModernWeeklySchedule() {
        ...
        if (facility.getWeeklyCalendar() == null) return;
        ...
        for (DailySchedule ds : sortedDays) {
            HBox row = new HBox();
            row.setAlignment(Pos.CENTER_LEFT); row.setSpacing(10);
            ...
            lblDay.setPrefWidth(50); lblDay.setStyle("-fx-font-weight: bold; -fx-text-fill: #555; -fx-font-size: 16px;");
            Label lblTime = new Label(); lblTime.setStyle("-fx-font-size: 16px; -fx-font-family: 'Segoe UI';");
            boolean isClosed = ds.getIsClosed() != null && ds.getIsClosed();
            if (isClosed) { lblTime.setText("Closed"); lblTime.setStyle(lblTime.getStyle() + "-fx-text-fill: #e74c3c;"); }
            else { lblTime.setText(ds.getOpenTime() + " - " + ds.getCloseTime()); lblTime.setStyle(lblTime.getStyle() + "-fx-text-fill: #27ae60;"); }
    ...
    private void handleDateSelection(LocalDate date) {
        ...
        startTimeDisplay.setText("Select");
        endTimeDisplay.setText("Select");
        ...
        if (date.equals(LocalDate.now()) && now.isAfter(openTime)) {
            ...
            if (loopStart.isAfter(closeTime) || loopStart.equals(closeTime)) {
                showError("Facility is closed for the rest of the day.");
                return;
            }
        }
    ...
    private void checkAndShowRecommendation(LocalDate date) {
        ...
        if (recommendation != null && suggestionBox != null) {
            try {
                ...
                lblSuggestionText.setText("Least busy time: " + recommendation);
    ...
    @FXML public void openStartTimeOverlay() {
        if (startOptions.isEmpty() || selectedDate == null) return;
        currentMode = TimeMode.START; overlayTitleLabel.setText("Select Start Time");
        populateOverlayTimes(startOptions, selectedStartTime); showTimeOverlay();
    }
    @FXML public void openEndTimeOverlay() {
        if (endOptions.isEmpty() || selectedDate == null) return;
        currentMode = TimeMode.END; overlayTitleLabel.setText("Select End Time");
        populateOverlayTimes(endOptions, selectedEndTime); showTimeOverlay();
    }
    ...
    private void populateOverlayTimes(List<LocalTime> options, LocalTime current) {
        ...
        for (LocalTime time : options) {
            ...
            btn.setPrefHeight(88); btn.setPrefWidth(CARD_WIDTH); btn.setMaxWidth(CARD_WIDTH);
    ...
    private void showTimeOverlay() { timeOverlay.setVisible(true); timeOverlay.setManaged(true); mainLayout.setEffect(new GaussianBlur(5)); }
    @FXML public void hideTimeOverlay() { timeOverlay.setVisible(false); timeOverlay.setManaged(false); if(mainLayout != null) mainLayout.setEffect(null); }
    private void updateStartDisplay() { startTimeDisplay.setText(selectedStartTime != null ? selectedStartTime.format(timeFormatter) : "Select"); }
    private void updateEndDisplay() { endTimeDisplay.setText(selectedEndTime != null ? selectedEndTime.format(timeFormatter) : "Select"); }

    @FXML public void handleNext() {
        if (selectedDate == null) { showError("Please select a day."); return; }
        if (selectedStartTime == null || selectedEndTime == null) { showError("Please select a valid time range."); return; }
        if (selectedDate.equals(LocalDate.now()) && selectedStartTime.isBefore(LocalTime.now())) { showError("Cannot select a time in the past."); return; }
        ...
        if (durationMinutes < 60) { 
            showError("Minimum reservation duration is 1 hour."); 
            return; 
        }
        if (durationMinutes > 180) { 
            showError("Maximum reservation duration is 3 hours."); 
            return; 
        }
    ...
    @FXML public void handleBack() { userHomeController.setView("/fxml/user/facility/user-facility-dashboard.fxml"); }
